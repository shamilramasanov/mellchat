  ...
  window.addEventListener('load', async () => {
    ...
    const registerSW = async (swPath) => {
      try {
        ...
        if (!response.ok) {
          console.warn(`⚠️ Service Worker file ${swPath} returned status ${response.status}`);
          return false;
        }
        ...
        if (text.trim().startsWith('<!') || text.trim().startsWith('<html') || text.trim().startsWith('<!DOCTYPE')) {
          console.warn(`⚠️ Service Worker file ${swPath} is HTML, not JavaScript`);
          return false;
        }
        
        // Проверяем content-type
        if (!contentType?.includes('javascript') && !contentType?.includes('application/javascript') && !contentType?.includes('text/javascript')) {
          console.warn(`⚠️ Service Worker file ${swPath} has wrong content-type: ${contentType}`);
          return false;
        }
        ...
        // Тихая обработка ошибок - не логируем как критичные
    ...
    // НЕ используем sw.js - он может возвращать HTML на Vercel
