...
class DatabaseService {
  ...
  async getMessages(streamId, limit = 100, offset = 0) {
    return this.request(`/api/v1/database/messages/${streamId}?limit=${limit}&offset=${offset}`);
  }

  // Get questions from database for a specific stream
  async getQuestions(streamId, limit = 50, offset = 0) {
    return this.request(`/api/v1/database/questions/${streamId}?limit=${limit}&offset=${offset}`);
  }

  // Search messages in database
  async searchMessages(streamId, searchQuery, limit = 50, offset = 0) {
    return this.request(`/api/v1/database/search/${streamId}?q=${encodeURIComponent(searchQuery)}&limit=${limit}&offset=${offset}`);
  }

  // Get stream statistics
  async getStreamStats(streamId) {
    return this.request(`/api/v1/database/stats/${streamId}`);
  }

  // Test database connection
  async testConnection() {
    return this.request('/api/v1/database/health?v=' + Date.now());
  }
}
