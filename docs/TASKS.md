...
### 6. Удаление светлой темы

**Задача:** Удалить поддержку светлой темы из приложения. Оставить только тёмную тему.

**Текущее поведение:**
- В приложении есть поддержка светлой темы
- Есть переключение между светлой и тёмной темой
- CSS переменные для светлой темы определены в `globals.css`
- Логика переключения тем в компонентах

**Требуемое поведение:**
- Удалить все CSS стили для светлой темы (body.light-theme)
- Удалить логику переключения тем из компонентов
- Удалить переключатель темы из настроек (если есть)
- Оставить только тёмную тему

**Файлы для изменения:**
- `frontend/pwa/src/styles/globals.css` - удалить CSS переменные для светлой темы
- `frontend/pwa/src/app/App.jsx` - удалить логику добавления класса light-theme
- `frontend/pwa/src/features/settings/components/SettingsPanel.jsx` - удалить переключатель темы и логику

**Детали реализации:**
- Удалить селектор `body.light-theme` и все связанные стили из `globals.css`
- Удалить код, который добавляет/удаляет класс `light-theme` из body
- Удалить UI элементы для переключения темы (если есть в настройках)
- Убедиться, что везде используется только тёмная тема
- Очистить localStorage от настроек темы (если они сохраняются)
